...
#define FirmwareVersion 309    // Firmware version (x100) used by OLED and GUI About.
...
uint64_t displacement1 = 0;
uint64_t displacement2 = 0;
uint64_t displacement3 = 0;

uint64_t previous_displacement1 = 0;
uint64_t previous_displacement2 = 0;
uint64_t previous_displacement3 = 0;
...
#define NUMBER_OF_DATAPOINTS_TO_AVERAGE
// Declare arrays to hold the last NUMBER_OF_DATAPOINTS_TO_AVERAGE displacement values
double displacement1_array[10] = {0};
double displacement2_array[10] = {0};
double displacement3_array[10] = {0};
...
// Load counter values
  ...
  // Average calculation
  displacement1_avg /= 10.0;
  displacement2_avg /= 10.0;
  displacement3_avg /= 10.0;
...
void USBSender()
{
  ...
  // XXX important: these are not atomic operations and the avg can be changed in the middle of the copy. Copy again, if value has changed.
  uint64_t localDisplacement1 = displacement1;
  while  (localDisplacement1 != (displacement1))
  {
    localDisplacement1 = displacement1;
  }
  double localDisplacement2 = displacement2_avg*100.0;
  double localDisplacement3 = displacement3_avg*100.0;
  if (Serial.availableForWrite() > 256)
  {
    Serial.printf("%li ", REF);                     // 1: REF
    Serial.printf("%li ", MEAS1);                   // 2: MEAS1
    Serial.printf("%li ", localDisplacement1);           // 3: Displacement 1
    Serial.printf("%li ", (uint64_t)velocity1);               // 4 Velocity Count 1
    ...
    if (Heterodyne > 1)
    {
      ...
      Serial.printf("%li ", (int64_t)localDisplacement2);       // 10: Displacement 2
      Serial.printf("%li ", (int64_t)velocity2);           // 11: Velocity Count 2
      Serial.print("0 ");                         // 12: Phase 2
      Serial.printf("%li ", MEAS3);               // 13: MEAS3
      Serial.printf("%li ", (int64_t)localDisplacement3);       // 14: Displacement 3
      Serial.printf("%li ", (int64_t)velocity3);           // 15: Velocity Count 3
...
// Turn off unused LEDs for MEAS2 and MEAS3 (and Homodyne channel 2) if no line receivers present.
  ...
  dataAqcuisitionTimer.begin(GrabAndOversampleData, 100);// send USB data every 100 microseconds
  dataAqcuisitionTimer.priority(199);                  // Lower numbers are higher priority, with 0 the highest and 255 the lowest. Most other interrupts default to 128
}

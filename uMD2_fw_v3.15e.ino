...
//        MEAS2 TMR1 D10 Pin 1U2 (MEAS2 was TMR2 D0 Pin 2, D10 was MEAS1)
//        MEAS3 TMR3 D14 Pin 16

// V3.15c - First public release of firmware with averaging / supersampling.
// V3.15d - Cleaned up somewhat, should be functionally the same as V3.15c.
// V3.15e - Use exponential moving average instead of straight average.
...
#define NumberSamples 250      // Weighting parameter for the moving average.  The default is 250.  Higher values will
                               //  result in more smoothing and a lower frequency response.
                               
#define AveragingRate 250      // Number of averaging updates per sample period.  The default is 250.  If the value is set too high
                               //  the Teensy may get into a funny mode requiring the Windows process to be killed and then the
                               //  Teensy to be power cycled or reset to be able to upload a new sketch.
...
int N = NumberSamples; // Weighting parameter of moving average
double NF = N;
...
// Load counter values
  ...
  // Average calculation
  displacement1_avg = (displacement1_avg * (NF-1.0)/NF) + (displacement1 / NF);  
  displacement2_avg = (displacement2_avg * (NF-1.0)/NF) + (displacement2 / NF);
  displacement3_avg = (displacement3_avg * (NF-1.0)/NF) + (displacement3 / NF);
...
// Turn off unused LEDs for MEAS2 and MEAS3 (and Homodyne channel 2) if no line receivers present.
  ...
  dataAqcuisitionTimer.begin(GrabAndOversampleData, 1000/AveragingRate);   // Average updated every 1000/AveragingRate microseconds.
